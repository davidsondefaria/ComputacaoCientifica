...
void gaussSeidel(){
	...
	while(dv>erro){
		...
		for (j=1; j<=(int)(tamY/2); j++){
			for(i=1; i<=(int)(tamX/2); i++){
				vn = pot[i][j];
				/*sif (fabs(pot[i][j]) == v1 && fabs(pot[i][j]) == v2){
					potNew[i][j] = pot[i][j];
...
void main(){
	...
	potencial = fopen("potFQ-Jacobi.dat", "w");
	campo = fopen("campoFQ-Jacobi.dat", "w");
	jacobi();
	//escreve pot
	for (j=0; j<tamY; j++){
		for (i=0; i<tamX; i++){
	...
	iniPot();
	fioQuad();
	potencial = fopen("potFQ-GaussSeidel.dat", "w");
	campo = fopen("campoFQ-GaussSeidel.dat", "w");
	gaussSeidel();
	//escreve pot
	for (j=0; j<tamY; j++){
		for (i=0; i<tamX; i++){
	...
	iniPot();
	fioQuad();
	potencial = fopen("potFQ-GaussSeidelSOR.dat", "w");
	campo = fopen("campoFQ-GaussSeidelSOr.dat", "w");
	gaussSeidelSOR(0.9);
	//escreve pot
	for (j=0; j<tamY; j++){
		for (i=0; i<tamX; i++){
