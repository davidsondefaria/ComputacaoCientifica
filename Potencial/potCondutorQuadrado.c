...
void main(){
	...
	iniPot();
	quadradoCondutor();
	potencial = fopen("potCQ-GaussSeidel.dat", "w");
	campo = fopen("campoCQ-GaussSeidel.dat", "w");
	gaussSeidel();
	//escreve pot
	for (j=0; j<tamY; j++){
		for (i=0; i<tamX; i++){
	...
	iniPot();
	quadradoCondutor();
	potencial = fopen("potCQ-GaussSeidelSOR.dat", "w");
	campo = fopen("campoCQ-GaussSeidelSOr.dat", "w");
	gaussSeidelSOR(1.9);
	//escreve pot
	for (j=0; j<tamY; j++){
		for (i=0; i<tamX; i++){
